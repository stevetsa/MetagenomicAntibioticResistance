...
# makeblastdb -in chr21.fa -dbtype nucl -out chr21
# 


if [ "$#" -ne 6 ]; then
	echo "Not Enough Arguments. Please enter sh nastyBugs.sh [SRA Directory] [Host Genome Directory] [Card Database Directory] [Variant Database Directory] [Available Cores] [Output Dire
ctory]"
fi
...
for sra in $FILES; do
  ...
  magicblast -sra $sra -db $hostGen -num_threads $cores -score 50 -penalty -3 -out $Out_DIR/$sra.human.sam
#  sed -n '1~4s/^@/>/p;2~4p' $indir/1.fastq > $indir/1.fasta
#  sed -n '1~4s/^@/>/p;2~4p' $indir/2.fastq > $indir/2.fasta
#  magicblast -num_threads $cores -db $hostGen -infmt fasta -query $indir/1.fasta -query_mate $indir/2.fasta -score 50 -penalty -3 -out $OUT_DIR/$sra.human.sam
  ...
  if [[ ($read0_count -lt $read1_count) && ($read1_count -eq $read2_count) ]]; then
    ...
    # should we sync the FASTQs here?
	  magicblast -num_threads $cores  -infmt fasta -query $tmp_read_one_trimmed -query_mate $tmp_read_two_trimmed -score 50 -penalty -3 -out $OUT_DIR/$sra.CARD_gene.sam -db $card
  elif [[ ($read1_count -lt $read0_count) && (-s $read0_count) ]]; then
	  fastx_clipper -i $tmp_read_zero -o $tmp_read_zero_trimmed
	  magicblast -num_threads $cores -infmt fasta -query $tmp_read_zero_trimmed -score 50 -penalty -3 -out $OUT_DIR/$sra.CARD_gene.sam -db $card 
  else 
  ...
  if [[ ($read0_count -lt $read1_count) && ($read1_count -eq $read2_count) ]]; then
    magicblast -num_threads $cores -infmt fasta -query $tmp_read_one_trimmed -query_mate $tmp_read_two_trimmed -score 50 -penalty -3 -out $OUT_DIR/$sra.CARD_variant.sam -db $card
  elif [[ ($read1_count -lt $read0_count) && (-s $read0_count) ]]; then
